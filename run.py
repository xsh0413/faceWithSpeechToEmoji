...
# 每次运行get_emoji，列表首元素会被弹出，本次分类的结果会放入列表末尾。
# 返回列表中出现次数最多的表情
# 但是带来了一些延迟
emojis = [0] * 10
    ...
    while myWin.isVisible():
        if myWin.flag_:
            ...
            if face is not None:
                ...
                # 提供一个默认图像或提示
                info = io.imread('null.png')
                info = cv2.cvtColor(info, cv2.COLOR_RGB2BGR)
                face_emoji = concatenate_images(frame, info, info, 10)
                myWin.getFaceemoji(face_emoji)
